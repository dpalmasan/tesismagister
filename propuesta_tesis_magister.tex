...
\author{Profesor: John Atkinson\\Alumno: Diego Andrés Palma Sánchez}
\date{Mayo de 2016}
...
Se propone desarrollar un método computacional que permita evaluar automáticamente ensayos considerando aspectos de coherencia textual y que considere la sintaxis y semántica.

Para ello se considerarán métodos que evalúen diferentes componentes de la coherencia textual, para posteriormente ensamblarlos.

El primer método a considerar será un método basado en características estadísticas de los textos, tal como el descrito en \cite{t40}. Este método de evaluación de ensayos considerará las características mostradas en la tabla \ref{t2}. Para aplicar este método, es necesario tener un conjunto de ensayos pre-evaluados, y ajustar un modelo de regresión como el mostrado en la ecuación \ref{eq0}.

Por otro lado, también se considerará un método basado en un modelo de espacio vectorial. Se utizará LSA, y se aplicará a un conjunto de ensayos pre-evaluados con el fin de obtener un espacio semántico. Dentro de dicho espacio se compararán los ensayos utilizando la similitud coseno (ecuación \ref{eq2}).

Para considerar las características sintácticas de los textos se utilizará un método basado en la teoría del centrado como el mostrado en \cite{t50}. Para ello, se requerirá representar un documento como una matriz de entidades, donde cada entidad tiene un rol dentro de la oración, como por ejemplo sujeto (s), objeto (o) u otro rol que no sea sujeto ni objeto (x). En la figura \ref{figura3} se muestra un ejemplo de una matriz de entidades, donde las filas de la matriz representan las oraciones del texto, las columnas son las entidades dentro del texto y en cada celda de la matriz se encuentra el rol de la entidad. Luego, el análisis de coherencia se hará considerando transiciones de entidades entre oraciones. Cada transición tendrá una cierta probabilidad dependiendo de la estructura de la matriz. Por ejemplo en la figura \ref{figura3} la transición {\em [s-]} tiene una probabilidad de 0.08 (se calcula como la razón entre la frecuencia de ocurrencia de la transición (8) y la cantidad de transiciones de largo 2, en este caso 75). Se considera que ciertos patrones de transiciones tienden a ocurrir en textos coherentes. Luego, cada ensayo se representará como un vector, donde cada componente del vector corresponderá a la probabilidad de ocurrencia de una transición, un ejemplo de ello se muestra en la figura \ref{figura4}. Finalmente se aplicará regresión para encontrar un modelo que relacione las probabilidades obtenidas y la nota del ensayo.

Otro método que se utilizará para evaluar la coherencia textual estará basado en un modelo de {\em cadenas léxicas}. Este modelo toma como supuesto que un texto es coherente si se utilizan palabras que estén relacionadas semánticamente entre oraciones. Para aplicar este método, se requiere tener una representación de matemática del significado de las palabras y así poder cuantificar la similitud entre ellas. Esto se logrará de dos formas alternativas, la primera es considerando un corpus de millones de documentos, como el corpus TASA (Touchstone Applied Science Associates). Este corpus contiene millones de documentos que consideran conceptos de una amplia cantidad de tópicos, y el significado de las palabras se extraerá utilizando LSA. Al aplicar LSA, se obtendrá un espacio semántico, y cada palabra estará representada como un vector. Luego, se podrá utilizar la similitud coseno para ver qué tan similares son dos palabras. Luego, cada texto se segmentará en oraciones, y para obtener una representación vectorial de estas, se considerará la suma vectorial de las palabras que la componen. Posteriormente, se calculará la similitud promedio entre oraciones adyacentes (ecuación \ref{eq5}). Alternativamente, se considerará otra representación de palabras como la realizada en \cite{t51}, pues esta a diferencia de LSA no asume un modelo de bolsa de palabras. Con los vectores de palabras obtenidos, se realizará el cálculo de similitud entre oraciones adyacentes como el descrito previamente.

Finalmente, utilizando estos métodos, se hará un ensamble (inicialmente con un modelo de regresión), es decir, se evaluará un texto considerando las componentes de la coherencia de todos los métodos.

\begin{table}[!htbp]
\centering
\caption{Atributos Lingüísticos y de Contenido}
\label{t2}
\begin{tabular}{lll}
\hline
\hline
{\bf lexical sophistication} & {\bf grammar} \\
\hline
{\em number of characters}       & {\em Number of different POS tags} \\
{\em number of words}            & {\b Number of Each POS tag} \\
{\em number of long words}       & {\em adverb}         \\
{\em number of short words}      & {\em coordinating conjunction}\\
{\em most frequent word length}  & {\em numeral}         \\
{\em average word length}        & {\em determiner}         \\
{\em number of sentences}        & {\em existential there}  \\
{\em number of long sentences}   & {\em preposition}         \\
{\em number of short sentences}  & {\em adjective}         \\
{\em most frequent sentence length} & {\em superlative adjective}      \\
{\em average sentence length}    & {\em comparative adjective}         \\
{\em number of different words}  & {\em modal auxiliary}         \\
{\em number of stop words}       & {\em singular or mass common noun} \\
{\b Readability measures \cite{t46}, \cite{t47}} & {\em plural common noun}\\
{\em Gunning Fog Index}          & {\em singular proper noun}    \\
{\em Flesch Reading Ease}        & {\em preposition participle}  \\
{\em Flesch Kincaid Grade Level} & {\em predeterminer}         \\
{\em Dale Chall Readability formula} & {\em personal pronoun}   \\
{\em automated readability index} & {\em possessive pronoun}    \\
{\em Simple Measure of Gobbledygook} & {\em adverb}         \\
{\em LIX}       & {\em genitive marker}     \\
{\em Word Variation Index}       & {\em comparative adverb}         \\
{\em Nominal Ratio}              & {\em superlative adverb}         \\
{\b Lexical Diversity \cite{t48}}       & {\em verb - base form}    \\
{\em TTR}                        & {\em verb - past tense}         \\
{\em Guiraud's index}            & {\em verb - gerund/present participle}\\
{\em Yule's K}                   & {\em verb - past participle}         \\
{\em hapax legomena}             & {\em wh-determiner}     \\
{\em Advanced Giraud}            & {\em wh-pronoun}     \\
           & {\em possessive wh-pronoun}         \\
                      & {\em wh-adverb}         \\
\hline
{\bf Content} & \\
\hline
...
2. score point level for maximum cosine & \\
similarity over all score points & \\
3. cosine similarity with essays that & \\
have highest score point level & \\
4. pattern cosine \cite{t49} & \\
5. weighted sum of all cosine correlation values \cite{t49} & \\
...
\begin{figure}[!htbp]
  \begin{center}
    \leavevmode
    \fbox{\includegraphics[width=4in] {figure_3.png}}
  \end{center}
  \caption{Matriz de entidades.}
  \label{figura3}
\end{figure}
...
\begin{figure}[!htbp]
  \begin{center}
    \leavevmode
    \fbox{\includegraphics[width=4in] {figure_4.png}}
  \end{center}
  \caption{Representación de documentos como vectores de transicion de entidades}
  \label{figura4}
\end{figure}
