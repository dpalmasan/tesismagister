...
En este trabajo se propone desarrollar un método computacional que permita evaluar automáticamente ensayos considerando aspectos de coherencia textual y que considere la sintaxis y semántica.

El primer método a considerar está basado en un modelo de regresión\cite{t40}. Este método de evaluación de ensayos considerará las características mostradas en la tabla \ref{t2}.

Para considerar las características sintácticas de los textos se utilizará un método basado en la teoría del centrado \cite{t50}. Para ello, se representan los documentos como una matriz de entidades, donde cada entidad tiene un rol dentro de la oración, como por ejemplo sujeto (s), objeto (o) u otro rol que no sea sujeto ni objeto (x). En la figura \ref{figura3} se muestra un ejemplo de una matriz de entidades, donde las filas de la matriz representan las oraciones del texto, las columnas son las entidades dentro del texto y en cada celda de la matriz se encuentra el rol de la entidad. Luego, el análisis de coherencia se hará considerando transiciones de entidades entre oraciones. Cada transición tendrá una cierta probabilidad dependiendo de la estructura de la matriz. Por ejemplo en la figura \ref{figura3} la transición {\em [s-]} tiene una probabilidad de 0.08 (se calcula como la razón entre la frecuencia de ocurrencia de la transición (8) y la cantidad de transiciones de largo 2, en este caso 75).

Otro método que se utilizará para evaluar la coherencia textual estará basado en un modelo de {\em cadenas léxicas}. Este método toma como supuesto que un texto es coherente si se utilizan palabras que estén relacionadas semánticamente entre oraciones. 
...
Las principales contribuciones de este trabajo son dos:

\begin{itemize}
	\item Se aplicará un ensamble de diferentes métodos basados en sintaxis y en semántica de los ensayos. La diferencia con las investigaciones recientes es que la tendencia apunta a aplicar nuevos métodos de clasificación a un conjunto de características estadísticas, o generar nuevas características que evalúen coherencia textual. Por otro lado, este trabajo apunta a utilizar técnicas de evaluación de coherencia textual (ej: modelo de cadenas léxicas, y teoría del centrado) que han sido utilizadas en otros contextos (generación de resúmenes, traducción automática), y se usarán para la evaluación automática de ensayos.
	
	\item Se realizará una comparación del rendimiento de los métodos para evaluar coherencia textual, en la evaluación automática de ensayos y otros algoritmos de aprendizaje automático utilizados para la evaluación automática de ensayos, lo que dará nuevas direcciones para investigar este problema.
\end{itemize}
...
\begin{enumerate}
	...
	\item Recopilación de datos de ensayos evaluados por humanos, los cuales se limpiarán y prepararán para utilizarlos en el método a desarrollar. Para este procesamiento y limpieza se utilizarán herramientas existentes como por ejemplo nltk, pyEnchant, entre otros. Los datos a utilizar serán los proporcionados por Kaggle\footnote{http://www.kaggle.com/c/asap-aes/data} en la competencia {\em Automatic Essay Scoring}, el cual contiene datos de ocho categorías diferentes de ensayo, que del total cuatro consisten en géneros de escritura tradicional (persuasivo, narrativo, etc.) y los otros cuatro están basados en una fuente (es decir, los estudiantes leen un documento fuente y discuten preguntas respecto a dicho documento).
	
	\item Desarrollo de un método de evaluación que considere sintaxis y semántica del ensayo a evaluar. Para ello, se estudiarán modelos de evaluación de coherencia textual que se fundamenten en teoría de discursos, pues estos consideran estas dos componentes.
	
	\item Implementación de un prototipo computacional de los distintos métodos propuestos en la literatura, con la finalidad de realizar experimentos que validen la hipótesis. Se utilizarán herramientas y bibliotecas con algunos métodos ya implementados (por ejemplo: LSA), los cuales requerirán leves adaptaciones para aplicarlos al problema a investigar.
...
\begin{itemize}
	...
	\item Diseño de un método automático para evaluar ensayos: 13 Junio - 13 Agosto.
	\item Implementación de métodos en el estado del arte, para posteriormente comparar con lo propuesto: 13 Junio - 13 Septiembre
	\item Ensamble de modelos que consideren sintaxis y semántica: 13 Agosto - 13 Septiembre
	\item Crear prototipo para realizar pruebas 13 Junio - 13 Septiembre.
	\item Evaluar rendimiento del modelo y comparar con modelos del estado del arte: 13 Septiembre - 13 Octubre.
\end{itemize}
