...
Para disminuir los costos del personal requerido para revisar evaluaciones textuales a gran escala, se han propuesto métodos para evaluar textos de manera automática, tarea conocida como {\em Automatic Essay Scoring}. Los primeros sistemas propuestos para esta tarea intentan evaluar características relacionadas a la calidad del texto tales como la dicción, uso de vocabulario, coherencia, entre otras. Para ello, se utiliza la ocurrencia de términos (conteo de signos de puntuación, largo de las palabras, entre otros). 
...
La primeras técnicas de evaluación automática de textos (como PEG \cite{t0})  modelan los textos como una combinación lineal de sus características intrínsicas (dicción, contenido, fluidez, etc.). Estas características intrínsicas se estiman mediante características superficiales ({\em proxes}) tales como: la cantidad de palabras, largo del ensayo, cantidad de signos de puntuación utilizados, largo de las oraciones, etc. La metodología para la evaluación consta de dos etapas, una de entrenamiento y una de evaluación. En la etapa de entrenamiento se utilizan textos que ya tienen un puntaje asignado. Luego, existe una fase de estracción de características para posteriormente aplicar una regresión lineal (multi-variable), de manera de ajustar las ponderaciones (pesos) de cada característica. Finalmente, el puntaje de un ensayo se calcula como se muestra en la ecuación \ref{eq0}.
...
Otro problema que tiene LSA, además de la selección de dimensionalidad, es que no define una adecuada distribución de probabilidades, es decir, se obtienen tópicos como por ejemplo $0.4$ auto + 1.1 $camion$, lo cual podría interpretarse como el concepto de vehículo, sin embargo, pueden aparecer relaciones sin interpretación evidente. Otro problema, es que pueden aparecer valores negativos en la matriz término-documento. Para solventar estos problemas, se han propuesto métodos como PLSA ({\em Probabilistic Latent Semantic Analysis}) y LDA ({\em Latent Dirichlet Allocation}). Estos métodos lo que hacen es representar los documentos como mezclas de tópicos, los cuales están representados por una distribución de probabilidad de palabras. Por ejemplo el texto ``{\em El  hamster está comiendo brocoli}'' podría ser representado por dos tópicos (comida y animales).
